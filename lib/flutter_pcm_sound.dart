...
class FlutterPcmSound {
  ...
  static Future<void> feed(Int16List buffer) async {
    return await _invokeMethod('feed', {'buffer': buffer.buffer.asUint8List()});
  }
...
class MajorScale {
  ...
  List<int> sineWave(
    ...
    for (int i = 0; i < totalFrames; i++) {
      data[i] =
          (math.sin(step * (i % nFramesPerPeriod)) * volume * 32767).toInt();
    }
    return data;
  }
  ...
  List<int> generate({required int periods}) {
    ...
    for (int i = 0; i < periods; i++) {
      ...
      frames +=
          sineWave(periods: 1, sampleRate: sampleRate, freq: scale[noteIdx]);
      _periodCount++;
    }
    return frames;
  }
